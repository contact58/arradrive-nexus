      ...
      <div className="pt-28 pb-20 px-6">
        <div className="max-w-2xl mx-auto">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-12">
            <h1 className="sr-only">Réserver un taxi à Arras - Les Chauffeurs d'Arras</h1>
            ...
            {step === 0 && (
              <div className="space-y-5">
                <div>
                  ...
                  <input className={inputClass} placeholder="Ex: Gare d'Arras" value={form.pickup} onChange={(e) => update("pickup", e.target.value)} />
                </div>
                <div>
                  <label className="text-sm font-medium text-foreground mb-2 flex items-center gap-2">
                    <MapPin className="w-4 h-4 text-primary" /> Destination
                  </label>
                  <input className={inputClass} placeholder="Ex: Aéroport de Lesquin" value={form.destination} onChange={(e) => update("destination", e.target.value)} />
                </div>
              </div>
            )}
